@props([
    ...
    <a href="{{route('profile.manage')}}"
       class="px-3 py-1.5 rounded-full text-sm font-semibold transition-all duration-200 {{ request()->routeIs('profile.*')
            ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5'
            : 'text-slate-500 hover:text-slate-900 hover:bg-white/50' }}">
        Profil
    </a>
    ...
    <a href="{{ route('chat.index') }}"
       class="relative px-3 py-1.5 rounded-full text-sm font-semibold transition-all duration-200 {{ request()->routeIs('chat.*')
            ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5'
            : 'text-slate-500 hover:text-slate-900 hover:bg-white/50' }}">
        Messagerie
    </a>
