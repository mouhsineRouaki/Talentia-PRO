                ...
                if (isMe) {
                    ...
                    if (message.attach && message.attach.path) {
                        ...
                        if (message.attach.mime_type.startsWith('image/')) {
                            attachmentHtml = `<div class="mb-1"><a href="${path}" target="_blank" class="block overflow-hidden rounded-2xl border-4 border-white shadow-md transition-transform hover:scale-[1.02]"><img src="${path}" class="max-w-[240px] max-h-[240px] object-cover"></a></div>`;
                        } else {
                            attachmentHtml = `<div class="mb-1"><a href="${path}" target="_blank" class="flex items-center gap-3 bg-white p-3 rounded-2xl border border-indigo-100 shadow-sm hover:shadow-md transition-all max-w-[240px]"><div class="w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-500"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div><div class="flex-1 min-w-0 text-left"><p class="text-sm font-bold text-slate-800 truncate">${message.attach.filename}</p><p class="text-[10px] font-medium text-slate-500">${(message.attach.size / 1024).toFixed(1)} KB</p></div></a></div>`;
                        }
                    }

                    html = `<div class="flex items-end justify-end gap-3 group animate-in slide-in-from-right-5 fade-in duration-300"><div class="flex flex-col space-y-1 max-w-lg items-end">${attachmentHtml}${message.text ? `<div class="relative bg-gradient-to-br from-indigo-600 to-violet-600 px-5 py-3.5 rounded-[1.3rem] rounded-br-none shadow-lg shadow-indigo-500/20 text-white text-[15px] leading-relaxed">${message.text}</div>` : ''}<div class="flex items-center gap-1.5 pr-1 opacity-60"><span class="text-[10px] font-bold text-slate-400">Maintenant</span><svg class="w-3.5 h-3.5 text-slate-300" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg></div></div><img src="${img}" class="w-8 h-8 rounded-full object-cover shadow-sm ring-2 ring-white self-end mb-6"></div>`;
                } else {
                    let attachmentHtml = '';
                    if (message.attach && message.attach.path) {
                        let path = "/storage/" + message.attach.path;
                        if (message.attach.mime_type && message.attach.mime_type.startsWith('image/')) {
                            ...
                            attachmentHtml = `<div class="mb-1"><a href="${path}" target="_blank" class="flex items-center gap-3 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all max-w-[240px]"><div class="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-500"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg></div><div class="flex-1 min-w-0 text-left"><p class="text-sm font-bold text-slate-800 truncate">${message.attach.filename}</p><p class="text-[10px] font-medium text-slate-500">${(message.attach.size / 1024).toFixed(1)} KB</p></div></a></div>`;
                        }
                    }

                    html = `<div class="flex items-end gap-3 group animate-in slide-in-from-left-5 fade-in duration-300"><img src="${img}" class="w-8 h-8 rounded-full object-cover shadow-sm ring-2 ring-white self-end mb-6"><div class="flex flex-col space-y-1 max-w-lg">${attachmentHtml}${message.text ? `<div class="bg-white px-5 py-3.5 rounded-[1.3rem] rounded-bl-none shadow-sm text-slate-800 text-[15px] leading-relaxed border border-slate-100/50">${message.text}</div>` : ''}<span class="text-[10px] font-bold text-slate-400 pl-2">Maintenant</span></div></div>`;
                }
